...
(function() {
  ...
  // Scroll state (compact the pill padding slightly)
  window.addEventListener('scroll', () => {
    navbarEl.classList.toggle('scrolled', window.scrollY > 30);
  }, { passive: true });

  if (navPill && spotlight && indicator) {
    // Mouse spotlight glow follows cursor inside pill
    ...
    // Sliding indicator follows hovered item
    function moveIndicatorTo(el) {
      if (!el) { indicator.style.opacity = '0'; return; }
      const pillRect = navPill.getBoundingClientRect();
      const itemRect = el.getBoundingClientRect();
      indicator.style.left    = (itemRect.left - pillRect.left - 5 + navPill.scrollLeft) + 'px';
    ...
    navPill.addEventListener('mouseleave', () => {
      // On leave, slide back to active item if any
      const activeItem = navPill.querySelector('.nav-item.active');
    ...
    const sectionObserver = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          // Move indicator to new active item
          const activeItem = navPill.querySelector('.nav-item.active');
  ...
  if (expSection && horizTrack) {
    ...
    let maxScroll   = 0;
    let totalSlides = horizTrack.children.length;
    ...
    const calculateDimensions = () => {
      maxScroll = horizTrack.scrollWidth - window.innerWidth;
    ...
    window.addEventListener('scroll', () => {
      if (maxScroll <= 0) calculateDimensions(); // Fallback if it was 0
    ...
    class Particle {
      ...
      update() {
        ...
        if (distance < mouse.radius + this.size) {
          if (mouse.x < this.x && this.x < heroCanvas.width - this.size * 10) {
            this.x += 3;
          }
          if (mouse.x > this.x && this.x > this.size * 10) {
            this.x -= 3;
          }
          if (mouse.y < this.y && this.y < heroCanvas.height - this.size * 10) {
            this.y += 3;
          }
      ...
      for (let a = 0; a < particlesArray.length; a++) {
        for (let b = a; b < particlesArray.length; b++) {
          ...
          if (distance < (120 * 120)) {
            ...
            ctx.strokeStyle = 'rgba(255, 49, 49, ' + (opacityValue * 0.15) + ')';
            ctx.lineWidth = 1;
            ctx.beginPath();
  ...
  if (heroStats) {
    const statNums = document.querySelectorAll('.hero-stat');
    ...
    const animateCounters = () => {
      ...
      statNums.forEach(stat => {
        ...
        function updateCount(currentTime) {
          ...
          const currentVal = target * easeOutExpo;

          if (isDecimal) {
            numEl.innerText = currentVal.toFixed(1);
          } else {
            numEl.innerText = Math.floor(currentVal);
          }

          if (progress < 1) {
            requestAnimationFrame(updateCount);
          } else {
            if (isDecimal) numEl.innerText = target.toFixed(1);
            else numEl.innerText = target;
          }
        }
        requestAnimationFrame(updateCount);
      });
